...
from torch.npc_editor import (
    ...
    _generate_multi_state, _generate_multi_state_pages,
    _write_script_to_pory, _add_object_event,
    _add_bg_event, _remove_npc_from_map_json, _find_existing_nurse,
)
from torch.bulk_decompile import (
    find_script as _find_script,
    _extract_brace_block,
    classify_script as _classify_script,
    classify_inc_script as _classify_inc_script,
    strip_script_prefix as _strip_script_prefix,
    expand_battle_beats as _expand_battle_beats,
    inject_aliases_from_localids as _inject_aliases_from_localids,
    resolve_external_text_labels as _resolve_external_text_labels,
    _resolve_inc_text,
    decompile_inc_to_torscript as _decompile_inc_to_torscript,
    decompile_script_to_workspace as _decompile_script_to_workspace,
    decompile_mapscripts as _decompile_mapscripts,
)
...
def _assemble_npc_detail(obj, npc_id, gfx, script_label, script_type,
    ...
    return {
        ...
        "referenced_by": refs,
        "pages": _detect_pages(project_dir, map_name, script_label) if project_dir else [],
    }
...
def _detect_pages(project_dir, map_name, script_label):
    ...
    Returns list of {"page_num": int, "condition": str} dicts.
    """
                ...
                for line in f:
                    ...
                    if cmd == "page" and len(tokens) >= 2:
                        ...
                        if len(tokens) >= 4 and tokens[2] == "if":
                            cond = " ".join(tokens[3:])
                        current_page = {"page_num": pnum, "condition": cond}
...
def _create_multi_state(game_path, map_name, body, project_dir=None):
    """Create a multi-state NPC (object_event + script).

    When a workspace exists for this map, generates page-based TorScript
    in the workspace .txt file + auto-syncs. Otherwise, writes raw
    Poryscript to scripts.pory directly.
    """
    safe_name = _sanitize_label_name(body["name"])
    if not safe_name:
        return error_response("Name produces empty label after sanitization", 400)
    ...
    # Add object event first to get the NPC index
    obj_data = _build_object_data(body, label)
    new_idx = _add_object_event(game_path, map_name, obj_data)
    if new_idx is None:
        return error_response("Failed to write map.json", 500)

    # Check for workspace — use page-based TorScript if available
    ws_dir = os.path.join(project_dir, map_name) if project_dir else None
    if ws_dir and os.path.isdir(ws_dir):
        # Generate page-based TorScript
        script_txt = _generate_multi_state_pages(
            map_name, label, new_idx, states
        )
        txt_path = os.path.join(ws_dir, f"{safe_name}.txt")
        try:
