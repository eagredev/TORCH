...
# Ensure the parent directory (containing the torch package) is on sys.path
# so that `from torch.xxx import ...` resolves correctly regardless of
# where the package is installed (src/torch/, ~/torch_dev/, ~/torch_stable/).
_this_dir = os.path.dirname(os.path.realpath(os.path.abspath(__file__)))
_parent_dir = os.path.dirname(_this_dir)
_pkg_name = os.path.basename(_this_dir)  # e.g. "torch", "torch_dev", "torch_stable"
if _parent_dir not in sys.path:
    sys.path.insert(0, _parent_dir)
# If the package directory isn't named "torch", register it under that name
# so internal `from torch.X` imports work.
if _pkg_name != "torch":
    import importlib
    _pkg = importlib.import_module(_pkg_name)  # TORCH_PKG: dynamic
...
from torch import BUILD_TRACK as _BUILD_TRACK
_IS_DEV = _BUILD_TRACK == "dev"
